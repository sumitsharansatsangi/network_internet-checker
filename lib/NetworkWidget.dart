...
class _ConnectivityWidgetState extends State<ConnectivityWidget> {
  ...
  Future<void> _updateConnectionStatus(List<ConnectivityResult> result) async {
    setState(() {
      ...
      networkConnectivity = _connectionStatus.contains(ConnectivityResult.none) ? "ConnectivityResult.none" : "";
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SizedBox(
        ...
        widget.addIcon ?  widget.icon! :Image.asset("assets/no_network.jpg"),
            const SizedBox(height: 50),
            widget.noInternetText,
            const SizedBox(height: 30),
             isLoading ? Loading(loadingColor: widget.loadingColor,) :
            TextButton(
              onPressed: ()async{
